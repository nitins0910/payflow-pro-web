...
Sidebar me ek naya **Help & Support** icon (❓) add hua hai. User click karke subject + message likhta hai, "Send Message" dabata hai, aur woh seedha **payflowprosystem@gmail.com** par email ban jaata hai — koi ticket system nahi, bas ek email.
...
2. Function Gmail SMTP (Nodemailer) se `payflowprosystem@gmail.com` se hi email bhejta hai, khud ko hi, but `Reply-To` user ke email par.
...
1. `payflowprosystem@gmail.com` se sign in karke https://myaccount.google.com/security par jayein.
...
3. `payflowprosystem@gmail.com` ka inbox check karein (pehli baar **Spam/Promotions folder bhi check kar lein** — apne hi address se apne ko mail aane par kabhi-kabhi Gmail usko spam samajh leta hai; ek baar "Not Spam" mark karne ke baad theek ho jaata hai).
4. Reply karke check kar lein ki reply user ke email par ja raha hai (na ki wapas `payflowprosystem@gmail.com` par).
